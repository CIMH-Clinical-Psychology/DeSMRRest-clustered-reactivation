...
all_files <- list.files("./category", full.names = TRUE)
...
for (file in seq_along(learning_unit_files)) {
  ...
  units_df <- as.data.frame(matrix(nrow = nrow(learning_units), ncol = 10 * 6))
  new_names <- paste0(rep(c("item1_", "item2_", "choice1_", "choice2_", "choice3_", "correct_"), 10), rep(1:10, each = 6))
  ...
  for (i in seq_along(full_seq)) {
    ...
    all_triplets$item2[i] <- full_seq[ifelse((1 + i) <= 10, (1 + i), (1 + i) %% 10)]
    all_triplets$item3[i] <- full_seq[ifelse((2 + i) <= 10, (2 + i), (2 + i) %% 10)]
  }
